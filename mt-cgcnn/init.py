...
if __name__ == '__main__':
	...
	SAVEPATH = FILEPATH + 'results_IPR_new9/'
	...
	# Set the params here
	resume = '' #'pre-trained/3-prop/model_best.pth.tar'
	lr = 0.005
	decay = 0
	n_conv = 4
	n_hp = 1
	...
	best_error, test_error, test_error_vec, test_loss_vec = init(FILEPATH, SAVEPATH,
					...
					optimizer=optimizer, n_conv=n_conv, n_hp=n_hp, lr=lr, weight_decay=decay,
					workers=workers, metric=metric, scheduler=scheduler, batch_size=batch_size,
					atom_fea_len=atom_fea_len, h_fea_len=h_fea_len, seed=seed, weights=weights, dropout=dropout, resume=resume)
	if SAVE_RESULTS:
