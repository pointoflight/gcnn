...
def main(root_dir, save_dir, disable_cuda=True, workers=0, epochs=30,
	...
	if len(full_dataset) < 2000:
		warnings.warn('Dataset has less than 2000 data points. Lower accuracy is expected.')
	...
	path = 'pre-trained/19june/model_best.pth.tar'
	...
	if save_checkpoints:
		...
		titles = ['property_1', 'property_2']
		plotMultiGraph(np.array(train_error_vec_per_epoch), np.array(val_error_vec_per_epoch), label1='train error', label2='val error',
					path=args.save_dir, name='train_val_err_vs_epoch', titles=titles)
		...
		plotMultiGraph(np.array(train_loss_vec_per_epoch), np.array(val_loss_vec_per_epoch), label1='train loss', label2='val loss',
					path=args.save_dir, name='train_val_loss_vs_epoch', titles=titles)
		...
		plotGraph(train_loss_list, val_loss_list, path=args.save_dir, name='train_val_loss_avg_vs_epoch', label1='avg train loss', label2='avg val loss', title='all properties')
		...
		plotGraph(train_error_list, val_error_list, path=args.save_dir, name='train_val_err_avg_vs_epoch', label1='avg train error', label2='avg val error', title='all properties')
...
def train(train_loader, model, criterion, optimizer, epoch,
	...
	error_vector = AverageMeter(is_tensor=True, dimensions=[1,1])
	# loss_vector is an average loss we see per property. Its dim is (1,n_p)
	loss_vector = AverageMeter(is_tensor=True, dimensions=[1,1])
	...
	for i, (input, targets, _) in enumerate(train_loader):
		...
		if USE_WEIGHTED_LOSS:
			...
			# mse_loss = [criterion(output[:,i], targets_var[:,i]) for i in range(n_p-1)]# for individual properties
			mse_loss = [criterion(output[:,n_p-1], targets_var[:,n_p-1])] #for i in range(n_p-1)]
			# print("mse_loss =", mse_loss)
			loss_1 = torch.stack(mse_loss)
			# print("stack mse_loss", loss_1)
			loss_1 = torch.sum(loss_1)
			# print("sum mse_loss =", loss_1)
			# loss_1 = loss_1 / (n_p-1)
			# print("after divide", loss_1)
			# loss = np.sum(mse_loss) / n_p
		...
		losses.update(loss_1.data.item(), batch_size)
...
def mae(prediction, target):
	...
	"""
	n_p = target.shape[1]
	return FloatTensor([torch.sum(torch.abs(sanitize(prediction[:,n_p-1], target[:,n_p-1], return_diff=True)))])/target.shape[0]
				# for i in range(n_p-1)])/target.shape[0]
